...
const EditBundle = () => {
  ...
  const fetchBundle = async () => {
    try {
      ...
      const selected = bundle.items.map((item) => ({
      ...
      alert("Échec du chargement du bundle");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Bundle mis à jour avec succès!");
      navigate("/admin/bundles");
    } catch (error) {
      alert(
        error.response?.data?.message || "Échec de la mise à jour du bundle"
      );
    } finally {
      setLoading(false);
    }
  ...
  if (initialLoading) {
    return <LoadingSpinner size="lg" text="Chargement du bundle..." />;
  }
