...
const OrderDetails = () => {
  ...
  const verifyPayment = async (approved) => {
    ...
    if (!approved) {
      ...
      !window.confirm("Confirmer que le paiement a bien été reçu ?")
    ) {
      return;
    }
    ...
    try {
      ...
      await fetchOrderDetails();
      alert(approved ? "Paiement approuvé" : "Paiement rejeté");
    } catch (error) {
  ...
  const getNextStatuses = (currentStatus, currentOrder) => {
    const statusFlow = {
    ...
    let next = statusFlow[currentStatus] || [];

    // For orders requiring payment proof, block manual "confirmed" jump
    // until the payment has actually been verified — use the
    // Approve/Reject buttons in the Payment card instead.
    const needsProof = ["MOBILE_MONEY", "ORANGE_MONEY"].includes(
    ...
    if (
      ...
      next = next.filter((s) => s !== "CONFIRMED");
    }
              ...
              {order.paymentRejectionReason && (
                <div className="p-3 bg-red-50 rounded-lg">
                  <p className="text-sm text-red-600">
                    Motif du rejet : {order.paymentRejectionReason}
                  </p>
                </div>
              )}
              ...
              {order.paymentStatus === "PENDING_VERIFICATION" && (
                <div className="flex gap-2 pt-2">
                  ...
                  >
                    <div className="flex justify-center items-center">
                      <FiX />
                      <span className="ml-2">Rejeter</span>
                    </div>
                  </Button>
                </div>
              )}
